...
export default function ManageMarks() {
  ...
  const [students,  setStudents]  = useState([]);   // fetched after class selected
  const [entries,   setEntries]   = useState([]);   // [{ studentId, name, obtained, absent }]
  const [fetching,  setFetching]  = useState(false);
  const [saving,    setSaving]    = useState(false);
  const [step,      setStep]      = useState(1);    // 1 = meta form, 2 = marks table
  ...
  const loadStudents = useCallback(async () => {
    if (!meta.classNo) { toast.error('Select a class first'); return; }
              ...
              <motion.div key="step2"
                ...
                <div className="flex flex-wrap gap-3 mb-5 p-4 rounded-2xl border"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-white">{meta.exam}</span>
                  </div>
                  <div className="w-px h-4 bg-white/10 self-center" />
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">Class {meta.classNo}</span>
                  </div>
                  <div className="w-px h-4 bg-white/10 self-center" />
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">Subject:</span>
                    <span className="text-sm font-bold text-white">{meta.subject}</span>
                  </div>
                  <div className="w-px h-4 bg-white/10 self-center" />
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">Max:</span>
                    <span className="text-sm font-bold" style={{ color: '#f5c842' }}>{meta.maxMarks}</span>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-4 mb-4">
                  ...
                  <div className="flex gap-3 ml-auto flex-wrap">
                    ...
                    <span className="text-xs px-3 py-1 rounded-full bg-blue-900/30 border border-blue-700/30 text-blue-300">
                      ✅ {filled} filled
                    </span>
                    <span className="text-xs px-3 py-1 rounded-full bg-red-900/30 border border-red-700/30 text-red-300">
                      🚫 {absent} absent
                    </span>
                    {filled > 0 && (
                      ...
                      <div key={entry.studentId}
                          ...
                          <input
                            ...
                            className={`w-full px-3 py-2 rounded-lg text-sm font-mono border transition-colors focus:outline-none bg-[#0c1229] text-white ${
                              over
                                ...
                                : 'border-white/10 focus:border-[#f5c842]'
                        ...
                        <div className="col-span-2 flex items-center gap-1.5">
                          {!entry.absent && entry.obtained !== '' && (
                            <>
                              <span className="text-sm font-black" style={{ color: gradeColor[grade] || '#9ca3af' }}>
                                {grade}
                              </span>
                              {pct !== null && (
                        ...
                        <div className="col-span-2 flex justify-center">
                          <button
                            ...
                            className={`w-9 h-5 rounded-full transition-all relative ${
                              entry.absent ? 'bg-red-500' : 'bg-white/10'
                            }`}>
                            <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${
                              entry.absent ? 'left-[18px]' : 'left-0.5'
                            }`} />
                          </button>
                        </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handleBulkSave} disabled={saving || filled === 0}
                    ...
                    {saving
                      ? 'Saving...'
                      : `📊 Save Marks for ${filled} Student${filled !== 1 ? 's' : ''}`}
                  </button>
                </div>
              </motion.div>
            )}
